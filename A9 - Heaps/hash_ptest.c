...
HASHDATA tests[] = { { "a", 0 }, { "b", 1 }, { "c", 2 }, { "d", 3 }, { "e", 4 },
		{ "f", 5 }, { "g", 6 } };
...
void test_hahstable_insert() {
	...
	for (int i = 0; i < n; i++) {
		hashtable_insert(ht, tests[i].key, tests[i].value);
		printf("%s(%s %d): ", "hashtable_insert", tests[i].key, tests[i].value);
...
void test_hahstable_search() {
	...
	for (int i = 0; i < n; i++) {
		HASHNODE *hnp = hashtable_search(ht, search_tests[i]);
