...
HEAPDATA tests[] = { { 4, 10 }, { 5, 9 }, { 8, 6 }, { 7, 7 }, { 6, 8 },
		{ 12, 2 }, { 9, 5 } };
...
void test_heap_search_data() {
	printf("------------------\n");
	printf("Test: heap_search_data\n\n");
	printf("%s: ", "heap_search_data");
	display_heap(hp);
	int n = sizeof search_data_tess / sizeof *search_data_tess;
	for (int i = 0; i < n; i++) {
		int index = heap_search_data(hp, search_data_tess[i]);
		if (index >= 0)
